import { type AgentLog, AgentRole } from "@lynxkit/shared";
...
function mockGenerateTextImpl(opts: { prompt: string }): Promise<{
	text: string;
}> {
	const { prompt } = opts;
	let text = "{}";
	...
	if (
		prompt.includes("AI 能力") ||
		prompt.includes("AI 集成代码") ||
		prompt.includes("integration code")
	) {
		...
		prompt.includes("功能模块") ||
		prompt.includes("product manager")
	) {
		text = MOCK_PM_JSON;
	} else if (prompt.includes("设计系统") || prompt.includes("design system")) {
		text = MOCK_DESIGNER_JSON;
	} else if (
		prompt.includes("部署清单") ||
		prompt.includes("deploy manifest")
	) {
...
function makeMockContext(overrides?: {
	...
	const deployer = hasDeployer
		...
		: {
				deploy: vi
					.fn()
					.mockResolvedValue({ url: "https://deploy.test/session-1" }),
			};
...
describe("Orchestrator 9 层流水线集成测试", () => {
	beforeEach(() => {
		...
		vi.mocked(generateText).mockImplementation(mockGenerateTextImpl as never);
		vi.mocked(streamText).mockImplementation(mockStreamTextImpl as never);
	});
	...
	it("IT-002：④⑤ 并行执行（PM 和 Designer 的 onProgress 交错）", async () => {
		...
		const pmEvents = progressEvents.filter(
			(e) => e.agent === AgentRole.PRODUCT_MANAGER,
		);
		const designerEvents = progressEvents.filter(
			(e) => e.agent === AgentRole.DESIGNER,
		);
		...
		const deployEvents = progressEvents.filter(
			(e) => e.agent === AgentRole.DEPLOY,
		);
