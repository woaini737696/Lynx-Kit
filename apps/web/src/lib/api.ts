 ...
 * api-client 单例
 *
 * Web 端通过 @lynxkit/api-client 调用 LynxKit Hono API（默认线上 https://miaox.lynxdo.com/api）。
 * JWT 从 @lynxkit/store 的 auth-store 读取并注入 Authorization 头。
 * 后端地址可通过 NEXT_PUBLIC_API_URL 环境变量覆盖（开发时本地启动 API 用 http://localhost:8787/api）。
 */

const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL ?? "https://miaox.lynxdo.com/api";
